...
- IAM roles for task execution role and task roles, [definition](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_execution_IAM_role.html)
...
- Registering the ECS service as the target for the input ALB, the traffic with the correct Host name will be pass through the ALB to this ECS service
...
## Examples
- [Example with DNS validation](./examples/complete/)
...
| <a name="input_dd_api_key_arn"></a> [dd\_api\_key\_arn](#input\_dd\_api\_key\_arn) | DD API key for integrating with Datadog | `string` | n/a | yes |
...
| <a name="input_shared_data"></a> [shared\_data](#input\_shared\_data) | Shared data for synchorizing with other modules | <pre>object({<br/>    aws_account_id = string<br/>    dd_agent_image = string<br/>    dd_site        = string<br/>  })</pre> | n/a | yes |
